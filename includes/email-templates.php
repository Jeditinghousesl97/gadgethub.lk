...
{
    $store   = getSetting('store_name', 'Gadget Hub');
...
<div class="wrap">
  <div class="hdr">
    <div>
      <div class="hdr-logo">GADGET HUB</div>
      <div class="hdr-sub">Computer Parts &amp; Electronics</div>
    </div>
  </div>
  $body
  <div class="ftr">
...
function emailOrderConfirmation(array $order, array $items): string
{
    ...
    $handlingLine = $handling > 0
        ...
        : '';
    $phoneAlt = $order['customer_phone_alt_display'] ? '<div class="info-row"><span class="info-label">Phone 2</span><span class="info-val">' . htmlspecialchars($order['customer_phone_alt_display']) . '</span></div>' : '';
    $emailRow = !empty($order['customer_email']) ? '<div class="info-row"><span class="info-label">Email</span><span class="info-val">' . htmlspecialchars((string)$order['customer_email']) . '</span></div>' : '';
    $cityRow = $order['customer_city_display'] ? '<div class="info-row"><span class="info-label">City</span><span class="info-val">' . htmlspecialchars($order['customer_city_display']) . '</span></div>' : '';
    $districtRow = $order['customer_district_display'] ? '<div class="info-row"><span class="info-label">District</span><span class="info-val">' . htmlspecialchars($order['customer_district_display']) . '</span></div>' : '';
    $addr  = $order['customer_address_display'] ? '<div class="info-row"><span class="info-label">Address</span><span class="info-val">' . nl2br(htmlspecialchars($order['customer_address_display'])) . '</span></div>' : '';
    $notes = $order['notes_display'] ? '<div class="info-row"><span class="info-label">Notes</span><span class="info-val">' . nl2br(htmlspecialchars($order['notes_display'])) . '</span></div>' : '';
    ...
    $pmMap = ['cod'=>'Cash on Delivery','bank_transfer'=>'Bank Transfer','whatsapp'=>'WhatsApp Order','payhere'=>'PayHere','koko'=>'KOKO'];
    $pmVal = $order['payment_method'] ?? '';
    $pmTxt = $pmMap[$pmVal] ?? ucfirst(str_replace('_',' ', (string)$pmVal));
    $pmRow = $pmTxt ? '<div class="info-row"><span class="info-label">Payment</span><span class="info-val">' . htmlspecialchars($pmTxt) . '</span></div>' : '';
    $bankMeta = is_array($order['payment_meta'] ?? null) ? $order['payment_meta'] : [];
    $bankLines = [];
    if ($pmVal === 'bank_transfer') {
        if (!empty($bankMeta['bank_name'])) $bankLines[] = 'Bank: ' . htmlspecialchars((string)$bankMeta['bank_name']);
        if (!empty($bankMeta['account_name'])) $bankLines[] = 'Account Name: ' . htmlspecialchars((string)$bankMeta['account_name']);
        if (!empty($bankMeta['account_number'])) $bankLines[] = 'Account No: ' . htmlspecialchars((string)$bankMeta['account_number']);
        if (!empty($bankMeta['branch'])) $bankLines[] = 'Branch: ' . htmlspecialchars((string)$bankMeta['branch']);
        if (!empty($bankMeta['instructions'])) $bankLines[] = nl2br(htmlspecialchars((string)$bankMeta['instructions']));
    }
    $bankTransferBox = $bankLines
        ? '<h3 style="font-size:14px;font-weight:700;margin:18px 0 12px;color:#111">Bank Transfer Details</h3><div class="info-box"><div class="info-row"><span class="info-val">' . implode('<br>', $bankLines) . '</span></div></div>'
        : '';
