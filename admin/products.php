...
$filterFreeDelivery = get('filter') === 'free_delivery';
...
$totalFreeDelivery = $db->query('SELECT COUNT(*) FROM products WHERE free_delivery=1')->fetchColumn();
?>
...
<div style="display:flex;gap:12px;margin-bottom:20px;flex-wrap:wrap">
  <a href="products.php" class="<?= !$catId&&!$search&&!$filterLow&&!$filterFreeDelivery?'stat-strip active':'stat-strip' ?>">
  ...
  <a href="products.php?filter=free_delivery" class="<?= $filterFreeDelivery?'stat-strip active':'stat-strip' ?>">
    <i class="fas fa-truck" style="color:var(--green)"></i> Free Delivery <strong><?= $totalFreeDelivery ?></strong>
  </a>
...
<div class="card" style="padding:14px 20px;margin-bottom:20px">
  <form method="GET" style="display:flex;gap:12px;align-items:center;flex-wrap:wrap">
    ...
    <?php if ($search || $catId || $filterLow || $filterFreeDelivery): ?>
...
<div class="card" style="padding:0;overflow:hidden">
  ...
  <div style="display:flex;align-items:center;justify-content:space-between;padding:14px 20px;border-top:1px solid var(--border)">
    ...
    <div style="display:flex;gap:6px">
      <?php
      $qs = http_build_query(array_filter(['search'=>$search,'category'=>$catId,'filter'=>get('filter')]));
