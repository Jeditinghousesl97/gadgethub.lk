...
if ($isEdit) {
    ...
    $selectedCategoryIds = getProductCategoryIds((int)$product['id']);
...
if (isPost()) {
    ...
    $categoryIds = array_values(array_unique(array_filter(array_map('intval', $_POST['category_ids'] ?? []), fn($id) => $id > 0)));
    $catId    = $categoryIds[0] ?? 0;
    $brand    = post('brand');
    $sku      = post('sku') ?: null;
    ...
    $weightKg = (float)post('weight_kg', '0');
    $freeDelivery = post('free_delivery','0') === '1' ? 1 : 0;
    ...
    if (!$catId) $errors[] = 'Please select at least one category.';
    if ($price <= 0) $errors[] = 'Price must be greater than 0.';
    if ($weightKg < 0) $errors[] = 'Weight cannot be negative.';
    ...
    if (!$errors) {
        try {
            if ($isEdit) {
                $db->prepare('UPDATE products SET name=?,slug=?,category_id=?,brand=?,sku=?,price=?,old_price=?,short_description=?,description=?,stock_qty=?,weight_kg=?,free_delivery=?,in_stock=?,badge=?,rating=?,is_featured=?,is_active=?,thumbnail=? WHERE id=?')
                   ->execute([$name,$slug,$catId,$brand,$sku,$price,$oldPrice,$shortDesc,$desc,$stock,$weightKg,$freeDelivery,$inStock,$badge?:null,$rating,$featured,$active,$thumbnail,$product['id']]);
                $productId = $product['id'];
            } else {
                $db->prepare('INSERT INTO products (name,slug,category_id,brand,sku,price,old_price,short_description,description,stock_qty,weight_kg,free_delivery,in_stock,badge,rating,is_featured,is_active,thumbnail) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)')
                   ->execute([$name,$slug,$catId,$brand,$sku,$price,$oldPrice,$shortDesc,$desc,$stock,$weightKg,$freeDelivery,$inStock,$badge?:null,$rating,$featured,$active,$thumbnail]);
                $productId = (int)$db->lastInsertId();
            }

            syncProductCategories((int)$productId, $categoryIds);
...
$brandList = $db->query('SELECT name FROM brands WHERE is_active=1 ORDER BY sort_order, name')->fetchAll();
    ...
    <div class="card">
      ...
      <div class="form-grid form-grid-2">
        ...
        <div class="form-group">
          <label class="form-label">Categories <span style="color:var(--red)">*</span></label>
          <div style="display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:10px;padding:12px;background:var(--bg-3);border:1px solid var(--border);border-radius:var(--r);max-height:220px;overflow:auto">
            <?php foreach ($cats as $c): ?>
              <label style="display:flex;align-items:center;gap:9px;font-size:13px;color:var(--text);cursor:pointer">
                <input type="checkbox" name="category_ids[]" value="<?= $c['id'] ?>" <?= in_array((int)$c['id'], $selectedCategoryIds, true) ? 'checked' : '' ?>>
                <span><?= htmlspecialchars($c['name']) ?></span>
              </label>
            <?php endforeach ?>
          </div>
          <span class="form-hint">The first selected category will be used as the primary category for labels and compatibility.</span>
        </div>
        <div class="form-group">
          <label class="form-label">Brand</label>
          <input type="text" name="brand" class="form-input" placeholder="e.g. Intel, AMD, ASUS" list="brandSuggestions"
            value="<?= val($product,'brand') ?>">
          <?php if ($brandList): ?>
          <datalist id="brandSuggestions">
            <?php foreach ($brandList as $b): ?>
              <option value="<?= htmlspecialchars($b['name']) ?>"></option>
            <?php endforeach ?>
          </datalist>
          <?php endif ?>
        </div>
    ...
    <div class="card">
      ...
      <div class="form-grid form-grid-2">
        ...
        <div class="form-group">
          ...
          <input type="number" name="weight_kg" class="form-input" min="0" step="0.001"
            value="<?= val($product,'weight_kg','0') ?>">
          <span class="form-hint">Used for district-based delivery fee calculation.</span>
        </div>
        <div class="form-group">
      ...
      <div class="form-grid" style="gap:14px">
        ...
        <div class="form-group">
          ...
          <select name="free_delivery" class="form-select">
            <option value="0" <?= ($product['free_delivery']??0)==0?'selected':'' ?>>No</option>
            <option value="1" <?= ($product['free_delivery']??0)==1?'selected':'' ?>>Yes</option>
          </select>
          <span class="form-hint">Ignores this product's weight and delivery charge during checkout.</span>
        </div>
      </div>
