...
<div class="ann-bar">
  <div class="ann-wrap">
    <div class="ann-track" id="gnxAnnTrack">
...
<header class="site-header" id="siteHeader">
  <div class="header-inner">
    ...
    <a href="index.php" class="site-logo">
      <img src="images/logo.jpg" alt="Gadget Hub">
    </a>
...
<div class="wish-drawer" id="wishDrawer">
  ...
  function renderWishDrawer() {
    ...
    body.innerHTML = items.map(item => `
      <div class="wish-item" data-id="${item.id}">
        ...
        <div class="wish-item-info">
          ...
          <div class="wish-item-price">${GadgetHubWishlist.fmt(item.price)}</div>
        </div>
    ...
    body.querySelectorAll('[data-action]').forEach(btn => {
      btn.addEventListener('click', () => {
        const item = GadgetHubWishlist.getItems().find(i => i.id === btn.dataset.id);
        if (!item) return;
        if (btn.dataset.action === 'cart') {
          if (window.GadgetHubCart) { GadgetHubCart.addItem(item); }
          ...
          GadgetHubWishlist.removeItem(item.name);
          renderWishDrawer();
        }
      });
    });
    ...
    if (moveAll) {
      moveAll.onclick = () => {
        ...
        GadgetHubWishlist.getItems().forEach(i => GadgetHubCart.addItem(i));
        if (window.GadgetHubShowCartPopup) window.GadgetHubShowCartPopup();
        GadgetHubWishlist.clear();
        renderWishDrawer();
      };
    }
  }
  ...
  function renderDrawer() {
    ...
    countEl.textContent = GadgetHubCart.getCount();
    totalEl.textContent = GadgetHubCart.fmt(GadgetHubCart.getTotal());
    if (waBtn) waBtn.href = GadgetHubCart.buildWaText() || '#';
      ...
      <div class="cart-item" data-id="${item.id}">
        ...
        <div class="cart-item-info">
          ...
          <div class="cart-item-row">
            <span class="cart-item-price">${GadgetHubCart.fmt(item.price)}</span>
    ...
    body.querySelectorAll('[data-action]').forEach(btn => {
      btn.addEventListener('click', () => {
        ...
        if (action === 'remove') GadgetHubCart.removeItem(id);
        else {
          const item = GadgetHubCart.getItems().find(i => i.id === id);
          if (!item) return;
          GadgetHubCart.updateQty(id, action === 'inc' ? item.qty + 1 : item.qty - 1);
        }
        renderDrawer();
      });
    });
  ...
  document.addEventListener('DOMContentLoaded', () => {
    if (window.GadgetHubCart)     GadgetHubCart.updateBadge();
    if (window.GadgetHubWishlist) GadgetHubWishlist.updateBadge();
  });
  ...
  (function loadCategories() {
    ...
    fetch(base + 'api/categories.php')
      ...
      .then(function (cats) {
        ...
        var desktopLimit = 12;
        var stripCats = (isDesktop && cats.length > desktopLimit) ? cats.slice(0, desktopLimit) : cats;
        ...
        if (isDesktop && cats.length > desktopLimit) {
          var extra = cats.length - desktopLimit;
          stripHtml += '<a href="shop.php" class="cat-strip-item cat-strip-more" title="' + extra + ' more categories">' +
                         '<div class="cat-icon-box"><i class="fas fa-ellipsis-h"></i></div>' +
                         '<span>View More</span>' +
                         '<em class="cat-more-badge">+' + extra + '</em>' +
                       '</a>';
        }
  ...
  (function loadSettings() {
    ...
    fetch(base + 'api/settings.php')
      ...
      .then(function (cfg) {
        ...
        // Announcement bar items (icon + text + optional link)
        if (Array.isArray(cfg.announcements)) {
          var annTrack = document.getElementById('gnxAnnTrack');
          if (annTrack) {
            var esc = function (v) {
              return String(v || '')
                .replace(/&/g, '&amp;')
            ...
            var linkOk = function (v) {
              return /^(https?:\/\/|\/|[a-z0-9_\-]+\.php(\?|$))/i.test(v || '');
            };
            var itemHtml = function (item) {
              var icon = esc((item && item.icon) || 'fas fa-bullhorn');
              var text = esc((item && item.text) || '').trim();
              var link = String((item && item.link) || '').trim();
              if (!text) return '';
              var inner = '<i class="' + icon + '"></i> ' + text;
              if (link && linkOk(link)) return '<a href="' + esc(link) + '">' + inner + '</a>';
              return '<span>' + inner + '</span>';
            };
            var items = cfg.announcements.map(itemHtml).filter(Boolean);
            if (items.length) annTrack.innerHTML = items.concat(items).join('');
          }
        }

        // Apply theme colors (sitewide CSS variables)
        if (cfg.theme && typeof cfg.theme === 'object') {
          var t = cfg.theme;
          var root = document.documentElement;
          var hex = function (v) { return typeof v === 'string' && /^#[0-9a-fA-F]{6}$/.test(v); };
          var set = function (name, val) { if (hex(val)) root.style.setProperty(name, val.toLowerCase()); };

          set('--primary', t.primary);
          set('--primary-lt', t.primary_lt);
          set('--accent', t.accent);
          set('--green', t.green);
          set('--wa', t.wa);
          set('--bg', t.bg);
          ...
          set('--bg-4', t.bg4);
          set('--card', t.card);
          set('--card-hover', t.card_hover);
          set('--border', t.border);
          set('--border-lt', t.border_lt);
          set('--text', t.text);
          set('--text-2', t.text2);
          set('--text-muted', t.text_muted);
          set('--text-dim', t.text_dim);

          // Keep gradients in sync with primary/accent
          if (hex(t.primary) && hex(t.accent)) {
            root.style.setProperty('--gold', 'linear-gradient(135deg,' + t.primary.toLowerCase() + ' 0%,' + t.accent.toLowerCase() + ' 100%)');
            root.style.setProperty('--gold-r', 'linear-gradient(135deg,' + t.accent.toLowerCase() + ' 0%,' + t.primary.toLowerCase() + ' 100%)');
          }
        }
      })
      .catch(function () {});
