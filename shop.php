...
foreach ($categories as $category) {
    if (($category['slug'] ?? '') === get('cat')) {
        $selectedCatName = (string)$category['name'];
        break;
    }
}
$shopSearchRaw = trim(get('q'));
$shopTitle = $selectedCatName !== '' ? ($selectedCatName . ' | Shop | Gadget Hub') : 'Shop | Gadget Hub';
$shopDescription = $selectedCatName !== ''
    ? ('Browse ' . $selectedCatName . ' products at Gadget Hub, including genuine computer parts, electronics, and accessories.')
    : 'Shop computer parts, electronics and accessories at GADGET HUB. Processors, RAM, GPUs, storage, monitors, peripherals and more.';
$shopCanonical = $selectedCatName !== '' ? ('shop.php?cat=' . urlencode(get('cat'))) : 'shop.php';
$shopRobots = $shopSearchRaw !== '' ? 'noindex,follow' : 'index,follow';
?>
<!DOCTYPE html>
<html lang="en">
<head>
...
(function () {
  ...
  function applyFilters() {
    ...
    let visible = cards.filter(c => {
      ...
      if (activeCat !== 'all' && !categories.includes(activeCat)) return false;
  ...
  const urlBrand = (params.get('brand') || '').toLowerCase();
  ...
  if (urlBrand) {
    const brandCb = Array.from(brandBoxes).find(cb => cb.value.toLowerCase() === urlBrand);
