...
<head>
  <?= renderSeoHead([
      'title' => 'Your Cart | Gadget Hub',
      'description' => 'Review the items currently in your Gadget Hub shopping cart.',
      'canonical' => 'cart.php',
    ...
    <div class="cart-page-layout" id="cartPageLayout">
      ...
      <div class="cart-summary-card" id="cartSummaryCard">
        ...
        <div class="cart-summary-actions">
          <a href="checkout.php" class="btn btn-primary" style="justify-content:center;background:var(--primary);border-color:var(--primary);color:#fff">
            <i class="fas fa-shopping-bag"></i> Place Order Online
          </a>
          <a id="waOrderBtn" href="#" target="_blank" rel="noopener" class="btn btn-green" style="justify-content:center;background:transparent;border:1px solid #25d366;color:#25d366">
    ...
    <div class="products-grid">
      ...
      <div class="product-card"
        ...
        <div class="product-img-area <?= $badge ? 'has-top-badge' : '' ?>">
          ...
          <?php endif ?>
          <?php if ($freeDelivery): ?>
            <span class="p-badge free-delivery">Free Delivery</span>
          <?php endif ?>
...
(function () {
  ...
  function renderCart() {
    ...
    const count   = GadgetHubCart.getCount();
    const total   = GadgetHubCart.getTotal();
    ...
    document.getElementById('summarySubtotal').textContent  = GadgetHubCart.fmt(total);
    document.getElementById('summaryTotal').textContent     = GadgetHubCart.fmt(total);
    if (waBtn) waBtn.href = GadgetHubCart.buildWaText(WA_NUMBER) || '#';
    ...
    tbody.innerHTML = items.map(item => `
      <div class="cart-row" data-id="${item.id}">
        ...
        <div class="cart-row-price">${GadgetHubCart.fmt(item.price)}</div>
    ...
    tbody.querySelectorAll('[data-action]').forEach(btn => {
      btn.addEventListener('click', () => {
        ...
        if (action === 'remove') {
          GadgetHubCart.removeItem(id);
        } else {
          const it = GadgetHubCart.getItems().find(i => i.id === id);
          if (!it) return;
          GadgetHubCart.updateQty(id, action === 'inc' ? it.qty + 1 : it.qty - 1);
        }
        renderCart();
      });
    });
  ...
  document.getElementById('cartClearBtn').addEventListener('click', () => {
    if (confirm('Clear all items from your cart?')) { GadgetHubCart.clear(); renderCart(); }
  });
